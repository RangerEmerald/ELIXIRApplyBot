...
async function limitcommandusage(message, Discord){
    if(message.member.roles.cache.find(r => r.name.toLowerCase() === "officer") || message.member.roles.cache.find(r => r.name.toLowerCase() === "captain")) return;
    let date = new Date();
    ...
    if(limitArray[message.author.id].commands >= 5){
        ...
        if(role && role2 && message.member){
            ...
            .then(setTimeout(() => {
                if(limitArray[message.author.id]){
                    ...
                    if(limitArray[message.author.id].time > (2**31-1)/1000){
                        limitArray[message.author.id].time = (2**31-1)/1000;
                    }
                }
        ...
        const reply = await message.reply(`The failure of sending an application to ELIXIR ${limitArray[message.author.id].commands} times has resulted in you being temp muted. This is your ${limitArray[message.author.id].brokenamount} warning. Try again in ${limitArray[message.author.id].time} seconds.`)
